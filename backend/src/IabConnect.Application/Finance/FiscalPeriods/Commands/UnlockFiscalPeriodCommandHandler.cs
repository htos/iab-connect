...
public sealed class UnlockFiscalPeriodCommandHandler
    : IRequestHandler<UnlockFiscalPeriodCommand, FiscalPeriodDto?>
{
    private readonly IFiscalPeriodRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAuditService _auditService;

    public UnlockFiscalPeriodCommandHandler(
    ...
    public async Task<FiscalPeriodDto?> Handle(UnlockFiscalPeriodCommand request, CancellationToken ct)
    {
        var period = await _repository.GetByIdAsync(request.Id, ct);
        if (period is null)
            return null;

        period.Unlock(request.UserName);
        ...
        await _auditService.LogActionAsync(
            ...
            $"Unlocked fiscal period {period.Name}",
