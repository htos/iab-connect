...
interface SponsorContractLinksProps {
  contractLinks: ContractLinkDto[];
  addLink: ReturnType<typeof useSponsorDetailMutations>["addLink"];
  removeLink: ReturnType<typeof useSponsorDetailMutations>["removeLink"];
}

// Contract-links section of the detail page (E22-S3). Local add-form state stays
// here; the add/remove mutations + cache update live in
// `useSponsorDetailMutations`. Markup + empty state (`sponsors.noLinks`)
// preserved from the god-page. The shared `ContractLink*` types come from
// `@/types/sponsors` (not the sponsors slice), per the E22-S2 type split.
export function SponsorContractLinks({
  contractLinks,
  addLink,
  removeLink,
}: SponsorContractLinksProps) {
      ...
      {showAddLink && (
        <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
          ...
          <div className="mt-3 flex justify-end">
            ...
            >
              {addLink.isPending ? t("common.loading") : t("sponsors.addLink")}
            </button>
          </div>
        </div>
      )}
                ...
                return (
                  <tr key={link.id}>
                    ...
                    <td className="py-3 text-right">
                      ...
                      >
                        {removing ? "..." : t("sponsors.removeLink")}
                      </button>
                    </td>
                  </tr>
                );
