// Health feature API (E27-S4). DEC-1 = A: WRAPS the existing `@/lib/api/health`
...
// `vi.mock("@/lib/api/health")` intercepting with ZERO transport-mock edits (A94).
//
// AC-8 (res.ok fix) is applied INSIDE the wrapped lib fn (`@/lib/api/health`),
...
import { getHealthDetail } from "@/lib/api/health";
