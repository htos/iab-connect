...
/**
 * Login Page content (E30-S2 features/system slice).
 * REQ-001: Login & Zugriff (Admin und Mitglieder)
 *
 * Body relocated verbatim from `app/login/page.tsx` (now a thin entry). The
 * NextAuth `signIn("keycloak", { callbackUrl })` flow, callbackUrl/error-param
 * handling, disabled-account modal derivation, the already-authenticated redirect
 * effect, the env-built Keycloak reset link, and the dev-credentials block are all
 * preserved byte-for-byte (pinned by the E30-S2 characterization net).
 */
