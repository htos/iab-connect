...
/**
 * Auth Error Page content (E30-S2 features/system slice).
 * REQ-001: Handles authentication errors.
 *
 * Body + `errorMappings` table relocated verbatim from `app/auth/error/page.tsx`
 * (now a thin entry). The 11 error-code → {titleKey, descKey} mapping, the
 * `?? "Default"` read-time fallback, the `?? errorMappings.Default` lookup
 * fallback, the /login + / links, and the dev debug block are unchanged (pinned
 * by the E30-S2 characterization net).
 */
