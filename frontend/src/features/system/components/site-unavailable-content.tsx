...
/**
 * Site-unavailable content (E30-S2 features/system slice).
 *
 ...
 *
 * Standalone minimal layout — `MainLayout` lists `/site-unavailable` as a full-page
 * route, so there is no authenticated shell and no public header/footer. It does NOT
 * adopt PageShell (full-page standalone frame, not the authenticated content frame).
 * Branding is read from the still-reachable anonymous `GET /api/v1/settings/public`;
 * if that fetch fails the page renders a plain unbranded message and never errors out.
 * Body relocated verbatim from `app/site-unavailable/page.tsx` (now a thin entry).
 */
...
export function SiteUnavailableContent() {
