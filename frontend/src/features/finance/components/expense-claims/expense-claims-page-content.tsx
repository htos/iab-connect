...
/**
 * Expense-claims content (E26-S3 migration of `app/finance/expense-claims/page.tsx`).
 ...
 * `return null`. SERVER filters `?status=&myClaimsOnly=` (search is client-side). Per-claim
 * ownership+role predicates preserved VERBATIM (claimantId===user?.email||isAdmin for
 * edit/submit/delete; kassier/vorstand for review/approve/reject/reimburse). All actions go
 * through a two-step confirm modal; reject reason `.trim()` is an enable-guard only. Confirm
 * button colours (A86): delete/reject=red, approve=green, reimburse=emerald, else orange.
 * Save + action reset/close driven from onSuccess (A92 — error keeps the modal open).
 */
...
function ExpenseClaimsBody() {
  ...
  const [statusFilter, setStatusFilter] =
    useState<ExpenseClaimStatusFilter>("all");
  ...
  const loading = canReadFinance && claimsQuery.isLoading;

  const saveMutation = useSaveExpenseClaim();
  const actionMutation = useExpenseClaimAction();
  ...
  const [reimburseMethod, setReimburseMethod] = useState<
    "Transfer" | "Cash" | "Online"
  >("Transfer");
  const [reimburseReference, setReimburseReference] = useState("");

  const submitting = saveMutation.isPending || actionMutation.isPending;

  // Load error surfaced via a DERIVED banner (not an effect — no setState-in-effect). A
  // manual action error takes precedence; otherwise the failed list query shows "error".
  const banner = error ?? (claimsQuery.isError ? t("error") : null);
  ...
  const handleSave = () => {
    const payload = { ...formData, receiptId: formData.receiptId || null };
    saveMutation.mutate(
      { editingId: editingClaim?.id ?? null, payload },
      {
        onSuccess: () => {
  ...
  // Per-claim ownership/role predicates (verbatim).
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] bg-gray-50 p-4 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {selectedClaim && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="max-h-[90vh] w-full max-w-2xl space-y-5 overflow-y-auto rounded-xl bg-white p-6 shadow-lg">
            ...
            <div className="w-full max-w-md space-y-4 rounded-xl bg-white p-6 shadow-lg">
              ...
              <div className="rounded-lg bg-gray-50 p-3 text-sm">
                <p className="font-medium text-gray-900">
                  {actionModal.claim.title}
                </p>
                <p className="text-gray-500">
                  {formatAmount(
                    actionModal.claim.amount,
                    actionModal.claim.currency
                  )}{" "}
                  · {formatDate(actionModal.claim.date)}
                </p>
              </div>
                ...
                <>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setReimburseMethod(
                          e.target.value as "Transfer" | "Cash" | "Online"
                        )
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-orange-500 focus:ring-orange-500"
                    >
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button
                  ...
                  disabled={
                    submitting ||
                    (actionModal.type === "reject" && !actionReason.trim())
                  }
...
export function ExpenseClaimsPageContent() {
