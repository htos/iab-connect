...
describe('volunteer service wrappers', () => {
  ...
  it('getEventVolunteerShifts hits the expected URL and returns parsed payload', async () => {
    const payload: EventVolunteerShiftDto[] = [
      {
        id: 's1', eventId: 'e1', roleId: 'r1', roleName: 'Greeter',
        title: 'Morning', description: null,
        startsAt: '2026-05-13T10:00:00Z', endsAt: '2026-05-13T12:00:00Z',
        capacity: 5, confirmedCount: 1, waitlistCount: 0,
        allowWaitlist: true, allowSelfSignup: true, notes: null,
  ...
  it('signUpForVolunteerShift surfaces 409 errorCode in errorBody', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ message: 'Shift full', errorCode: 'ShiftFull' }), {
  ...
  it('signUpForVolunteerShift surfaces 403 SignupNotAllowed', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ message: 'Self-signup is disabled', errorCode: 'SignupNotAllowed' }), {
  ...
  it('signUpForVolunteerShift returns AlreadyAssigned on the conflict path', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ message: 'Already assigned', errorCode: 'AlreadyAssigned' }), {
        status: 409, headers: { 'Content-Type': 'application/json' },
      })
    );
  ...
  it('withdrawFromVolunteerShift POSTs to the cancel sub-resource URL', async () => {
    ...
    const [calledUrl, calledInit] = fetchMock.mock.calls[0];
    expect(calledUrl).toBe(`${API_BASE}/api/v1/events/e1/volunteer-shifts/s1/assignments/a1/cancel`);
