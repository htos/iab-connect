 ...
 * Form for editing existing events - with i18n support
 */
'use client';
...
import { EventVisibility, EventCategory } from '@/lib/services/events';
...
export default function EditEventPage({ params }: EditEventPageProps) {
  ...
  const loadEvent = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/events/${resolvedParams.id}`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.message || t('errors.updateFailed'));
        setSaving(false);
      }
    } catch {
