...
export default function EditEmailCampaignPage() {
  ...
  const fetchCampaign = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}/api/v1/email-campaigns/${campaignId}`,
        {
  ...
  }, [accessToken, baseUrl, formData.segmentType, formData.segmentFilter]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || errorData.error || t("form.saveError")
        );
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4 pb-8">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex items-center gap-2 rounded-lg bg-orange-600 px-6 py-2.5 font-medium text-white transition-colors hover:bg-orange-700 disabled:opacity-50"
            >
              {saving ? (
                <>
