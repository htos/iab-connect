...
export default function AccountingReportsPage() {
  ...
  const fetchTrialBalance = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiRef.current.get(`/api/v1/finance/accounting-reports/trial-balance?${params}`);
      if (res.error) throw new Error(res.error);
      const data = res.data as TrialBalanceReport;
      setTrialBalance({ ...data, lines: data.lines ?? [] });
    } catch {
  ...
  const fetchBalanceSheet = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiRef.current.get(`/api/v1/finance/accounting-reports/balance-sheet?${params}`);
      if (res.error) throw new Error(res.error);
  ...
  const fetchProfitAndLoss = useCallback(async () => {
    ...
    try {
      ...
      if (from) params.append("from", from);
      if (to) params.append("to", to);
      const res = await apiRef.current.get(`/api/v1/finance/accounting-reports/profit-and-loss?${params}`);
      if (res.error) throw new Error(res.error);
  ...
  const handlePrint = () => {
    ...
    const dateRange = from && to ? `${from} – ${to}` : from ? `ab ${from}` : to ? `bis ${to}` : ta("allPeriods");
    ...
    if (activeTab === "trialBalance" && trialBalance) {
      ...
      const section = (title: string, rows: { accountNumber: string; accountName: string; balance: number }[], total: number) => {
        const body = rows
          .map((r) => `<tr><td>${r.accountNumber}</td><td>${r.accountName}</td><td class="right${r.balance < 0 ? " neg" : ""}">${fmtAmt(r.balance)}</td></tr>`)
          .join("");
      ...
      printReport(
        ...
        section(ta("assets"), balanceSheet.assets, balanceSheet.totalAssets) +
          section(ta("liabilities"), balanceSheet.liabilities, balanceSheet.totalLiabilities) +
          section(ta("equity"), balanceSheet.equity, balanceSheet.totalEquity)
      );
    } else if (activeTab === "profitAndLoss" && profitAndLoss) {
      const section = (title: string, rows: { accountNumber: string; accountName: string; amount: number }[], total: number) => {
        const body = rows
          .map((r) => `<tr><td>${r.accountNumber}</td><td>${r.accountName}</td><td class="right">${fmtAmt(Math.abs(r.amount))}</td></tr>`)
          .join("");
      ...
      printReport(
        ta("profitAndLoss"),
        dateRange,
        section(ta("revenue"), profitAndLoss.revenue, profitAndLoss.totalRevenue) +
          section(ta("expenses"), profitAndLoss.expenses, profitAndLoss.totalExpenses) +
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex items-end gap-4 rounded-xl bg-white p-4 shadow-sm">
          {activeTab === "balanceSheet" ? (
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">{ta("asOfDate")}</label>
              <input
                type="date"
                value={asOfDate}
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 text-gray-500">
                  ...
                  <th className="pb-3 text-right font-medium">{ta("totalDebitCol")}</th>
                  <th className="pb-3 text-right font-medium">{ta("totalCreditCol")}</th>
                  <th className="pb-3 text-right font-medium">{ta("balanceCol")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {trialBalance.lines.map((row) => (
                  <tr key={row.ledgerAccountId}>
                    ...
                    <td className="py-2 text-right font-mono">{formatCurrency(row.totalDebit)}</td>
                    <td className="py-2 text-right font-mono">{formatCurrency(row.totalCredit)}</td>
                    <td className={`py-2 text-right font-mono font-medium ${row.balance < 0 ? "text-red-600" : ""}`}>
              ...
              <tfoot>
                <tr className="border-t-2 border-gray-300 font-semibold">
                  <td colSpan={3} className="py-2">{t("total")}</td>
                  <td className="py-2 text-right font-mono">{formatCurrency(trialBalance.totalDebit)}</td>
                  <td className="py-2 text-right font-mono">{formatCurrency(trialBalance.totalCredit)}</td>
                  <td className="py-2 text-right font-mono">{formatCurrency(trialBalance.totalDebit - trialBalance.totalCredit)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        {activeTab === "balanceSheet" && balanceSheet && !loading && (
          <div className="space-y-6">
            {[
              { rows: balanceSheet.assets, title: ta("assets"), total: balanceSheet.totalAssets },
              { rows: balanceSheet.liabilities, title: ta("liabilities"), total: balanceSheet.totalLiabilities },
              { rows: balanceSheet.equity, title: ta("equity"), total: balanceSheet.totalEquity },
            ].map(({ rows, title, total }) => (
              <div key={title} className="overflow-x-auto rounded-xl bg-white p-6 shadow-sm">
                ...
                {rows.length === 0 ? (
                  ...
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="border-b border-gray-200 text-gray-500">
                        <th className="pb-2 font-medium">{ta("accountNumber")}</th>
                        <th className="pb-2 font-medium">{ta("accountName")}</th>
                        <th className="pb-2 text-right font-medium">{ta("balanceCol")}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {rows.map((row) => (
                        <tr key={row.ledgerAccountId}>
                          <td className="py-2 font-mono">{row.accountNumber}</td>
                          <td className="py-2">{row.accountName}</td>
                          <td className={`py-2 text-right font-mono ${row.balance < 0 ? "text-red-600" : ""}`}>
                            {formatCurrency(row.balance)}
                          </td>
                        </tr>
                      ))}
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-gray-300 font-semibold">
                        <td colSpan={2} className="py-2">{ta("sectionTotal")}</td>
                        <td className={`py-2 text-right font-mono ${total < 0 ? "text-red-600" : ""}`}>
        ...
        {activeTab === "profitAndLoss" && profitAndLoss && !loading && (
          <div className="space-y-6">
            {[
              { rows: profitAndLoss.revenue, title: ta("revenue"), total: profitAndLoss.totalRevenue },
              { rows: profitAndLoss.expenses, title: ta("expenses"), total: profitAndLoss.totalExpenses },
            ].map(({ rows, title, total }) => (
              <div key={title} className="overflow-x-auto rounded-xl bg-white p-6 shadow-sm">
                <h3 className="mb-3 text-lg font-semibold text-gray-900">{title}</h3>
                {rows.length === 0 ? (
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {rows.map((row) => (
                        <tr key={row.ledgerAccountId}>
                          <td className="py-2 font-mono">{row.accountNumber}</td>
                          <td className="py-2">{row.accountName}</td>
                          <td className="py-2 text-right font-mono">{formatCurrency(Math.abs(row.amount))}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr className="border-t-2 border-gray-300 font-semibold">
                        <td colSpan={2} className="py-2">{ta("sectionTotal")}</td>
                        <td className="py-2 text-right font-mono">{formatCurrency(Math.abs(total))}</td>
                      </tr>
                    </tfoot>
                  </table>
                )}
            ...
            <div className="rounded-xl bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-gray-900">{ta("netResult")}</h3>
                <span className={`text-2xl font-bold ${profitAndLoss.netResult >= 0 ? "text-green-600" : "text-red-600"}`}>
