...
export default function FinanceSettingsPage() {
  ...
  const [backfillResult, setBackfillResult] = useState<{
    ...
    errorCount: number;
    errors: { sourceType: string; sourceId: string; description: string; errorMessage: string }[];
    cutOffDate: string;
    executedAt: string;
  } | null>(null);
  ...
  const handleBackfill = async () => {
    ...
    try {
      const res = await apiRef.current.post("/api/v1/finance/backfill-double-entry", {
        cutOffDate: backfillDate || undefined,
      });
      if (res.error) throw new Error(res.error);
      setBackfillResult(res.data as typeof backfillResult);
    } catch (err) {
      setBackfillError(err instanceof Error ? err.message : t("settingsHub.backfillError"));
    } finally {
                ...
                <div className="mt-4 space-y-3">
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <div className="rounded-lg bg-green-50 p-3 text-center">
                      <div className="text-2xl font-bold text-green-700">{backfillResult.journalEntriesCreated}</div>
                      <div className="text-xs text-green-600">{t("settingsHub.backfillCreated")}</div>
                    </div>
                    <div className="rounded-lg bg-blue-50 p-3 text-center">
                      <div className="text-2xl font-bold text-blue-700">{backfillResult.transactionsProcessed}</div>
                      <div className="text-xs text-blue-600">{t("settingsHub.backfillTransactions")}</div>
                    </div>
                    <div className="rounded-lg bg-indigo-50 p-3 text-center">
                      <div className="text-2xl font-bold text-indigo-700">{backfillResult.paymentsProcessed}</div>
                      <div className="text-xs text-indigo-600">{t("settingsHub.backfillPayments")}</div>
                    </div>
                    <div className="rounded-lg bg-gray-50 p-3 text-center">
                      <div className="text-2xl font-bold text-gray-700">{backfillResult.skippedAlreadyPosted}</div>
                      <div className="text-xs text-gray-600">{t("settingsHub.backfillSkipped")}</div>
                    </div>
                  </div>
                  ...
                  {backfillResult.errorCount > 0 && (
                    <div className="rounded-lg border border-red-200 bg-red-50 p-3">
                      <h4 className="mb-2 text-sm font-semibold text-red-800">
                        {t("settingsHub.backfillErrors", { count: backfillResult.errorCount })}
                      </h4>
                      <div className="max-h-48 overflow-y-auto">
                        <table className="w-full text-xs">
                          <thead>
                            <tr className="border-b border-red-200 text-left text-red-700">
                              <th className="pb-1 pr-2">{t("settingsHub.backfillErrorType")}</th>
                              <th className="pb-1 pr-2">ID</th>
                              <th className="pb-1 pr-2">{t("settingsHub.backfillErrorDesc")}</th>
                              <th className="pb-1">{t("settingsHub.backfillErrorMessage")}</th>
                            </tr>
                          </thead>
                          <tbody>
                            {backfillResult.errors.map((err, i) => (
                              <tr key={i} className="border-b border-red-100">
                                <td className="py-1 pr-2 text-red-700">{err.sourceType}</td>
                                <td className="py-1 pr-2 font-mono text-red-600">{err.sourceId}</td>
                                <td className="py-1 pr-2 text-red-600">{err.description}</td>
                                <td className="py-1 text-red-600">{err.errorMessage}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
