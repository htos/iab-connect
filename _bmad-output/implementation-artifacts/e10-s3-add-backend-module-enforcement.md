...
| 2026-05-15 | Round-2 epic-boundary re-review: 3 ex-decisions + 1 patch applied. DN-3 → `ModuleAuthorizationHandler` fail-graceful 403 on infra failure (try/catch wraps `IsEnabledAsync`, audit `errorMessage` distinguishes module-disabled vs infra-crash). DN-2 → `ModulePolicyProvider` strict suffix validation against `ModuleKeys.All` + lowercase normalization (typo'd `Module:financ` now throws `InvalidOperationException` at first request). DN-4 → admin `BlogEndpoints` + admin `ContactEndpoints` chained onto `Module:communication`; `ModuleEnforcementEndpointTests.GatedEndpoints` theory extended (+2 rows × 2 tests = +4 cases). 1 defer added (pre-existing `EmailTemplate` bare-auth role-policy gap). Backend 1942/1942 green. Status → done. |
...
_E10-S3 routing: 3 decisions, 1 patch, 1 defer._

- [x] [Review][Patch] (was DN-3, Decision → Patch 2026-05-15) `ModuleAuthorizationHandler` exception from `_moduleSettings.IsEnabledAsync(...)` propagated → 500, inconsistent with the public filter's fail-open. **Chosen direction:** fail-graceful 403 [backend/src/IabConnect.Api/Authorization/ModuleAuthorizationHandler.cs:72-86] — **RESOLVED 2026-05-15:** `IsEnabledAsync` is now wrapped in `try/catch` — a transient cache/DB failure is logged (`LogError`), audited with a distinguishing errorMessage ("infrastructure failure"), and produces a clean 403 deny instead of a 500. Asymmetric to the public filter (fail-open) is intentional: fail-closed is the safer default for authenticated callers.
- [x] [Review][Patch] (was DN-2, Decision → Patch 2026-05-15) `ModulePolicyProvider` accepts any suffix after `"Module:"`, silently fail-open on typos. **Chosen direction:** strict [backend/src/IabConnect.Api/Authorization/PermissionAuthorizationHandler.cs:225-232] — **RESOLVED 2026-05-15:** `GetPolicyAsync` now lowercases the suffix and validates it against `ModuleKeys.All`; unknown suffix throws `InvalidOperationException("Unknown module key '<suffix>' in authorization policy '<full name>'. Module keys are case-insensitive and must be one of: …")` so misconfig fails-fast on the first request hitting the misconfigured endpoint. The runtime warn-log in `IsEnabledAsync` stays as belt-and-braces defense-in-depth.
- [x] [Review][Patch] (was DN-4, Decision → Patch 2026-05-15) Admin Blog and Contact endpoint groups carried only `RequireVorstand`. **Chosen direction:** chain `Module:communication` [backend/src/IabConnect.Api/Endpoints/BlogEndpoints.cs:659-695, backend/src/IabConnect.Api/Endpoints/ContactEndpoints.cs:1163-1192] — **RESOLVED 2026-05-15:** admin Blog (`/api/v1/blog`) and admin Contact (`/api/v1/contact-messages`) now chain `.RequireAuthorization("Module:communication")`. `ModuleEnforcementEndpointTests.GatedEndpoints` theory extended with two new rows (`/api/v1/blog/`, `/api/v1/contact-messages/`) covering both the disabled-→403 and enabled-→not-403 paths — 4 new test cases, all green.
- [x] [Review][Patch] `ModulePolicyProvider` case-**insensitive** prefix match + case-**sensitive** `IsEnabledAsync` dictionary lookup combined to silently fail-open on `Module:Finance` (capital F) [backend/src/IabConnect.Api/Authorization/PermissionAuthorizationHandler.cs:225-232] — **RESOLVED 2026-05-15:** folded into the DN-2 strict-validation patch — `GetPolicyAsync` lowercases the raw suffix (`rawSuffix.ToLowerInvariant()`) before building the requirement, so `Module:Finance` and `module:finance` both resolve to the canonical `"finance"` key.
- [x] [Review][Defer] `EmailTemplateEndpoints` chains bare `.RequireAuthorization()` (any authenticated user) + `.RequireAuthorization("Module:communication")` — the bare policy means non-Vorstand authenticated users can hit email-template endpoints when the module is on. Pre-existing role-policy gap not introduced by E10; tighten to `RequireVorstand` in a follow-up [backend/src/IabConnect.Api/Endpoints/EmailTemplateEndpoints.cs:1419-1424] — deferred
