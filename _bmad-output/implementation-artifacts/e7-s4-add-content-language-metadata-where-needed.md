...
I want public content to identify its language,
so that multilingual content can be managed cleanly.

## Acceptance Criteria

1. Given content manager has the required permission and prerequisite data exists, when public content to identify its language, then public content/event/blog records can identify content language where applicable.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then existing content continues to display with a default language.
3. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then public pages can filter or display language metadata without breaking current routes.
4. Given a caller lacks the required permission or role, when the protected behavior is requested, then content language changes are authorized through existing content management permissions.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then migration preserves existing data.
...
- Multi-epic sprint-plan order: 29.
...
- Adding language metadata to unrelated models
- Changing current public routes without compatibility

### Existing Code To Inspect Before Editing

- content/event/blog language metadata where product-approved
- EF migration if backend model changes
- public page display/filter support
...
- Build on `e7-s3-add-hindi-translation-expansion-path.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e7-s4-add-content-language-metadata-where-needed`.
