...
so that external systems can receive selected events.

## Acceptance Criteria

1. Given an Admin/IT user has the required permission and prerequisite data exists, when they configure webhooks, then admin can create, edit, disable, and delete webhook subscriptions.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then supported events include event created and payment received at minimum if product-approved.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then payloads are signed or otherwise verifiable.
4. Given valid inputs and existing system constraints, when this story behavior is exercised, then secret material is generated/stored safely.
5. Given the sensitive action succeeds or fails, when audit evidence is reviewed, then subscription changes are auditable.
...
- Multi-epic sprint-plan order: 32.
...
- Delivery retry worker/history
- Arbitrary event subscription

### Existing Code To Inspect Before Editing

- WebhookSubscription
- signing secret generation/storage
- event whitelist
- admin endpoints/UI
- audit logging
...
- Build on `e8-s2-add-read-api-endpoints.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e8-s3-add-webhook-subscriptions-and-signing`.
