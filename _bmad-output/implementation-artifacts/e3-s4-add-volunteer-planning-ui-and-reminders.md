...
Status: in-progress (Round-3 review complete 2026-05-14 — 0 Critical + 3 High + 4 Medium routed back to dev; 2 Defer logged)
...
- [ ] [Review][Patch] R3-H-S4-1 (High) `VolunteerShiftReminderService.ExecuteAsync` captures `nowUtc` once per batch — sample per-row immediately before `MarkReminderSentAsync` (BH-9) [backend/src/IabConnect.Application/Events/Jobs/VolunteerShiftReminderService.cs]
- [ ] [Review][Patch] R3-H-S4-2 (High) 36h reminder window + non-atomic mark-sent can double-send under transient SMTP outage — wrap send + mark in one transaction; also sync 24h→36h AC text (EC-7 + AA-16) [backend/src/IabConnect.Application/Events/Jobs/VolunteerShiftReminderService.cs:43-54, AC-5]
- [ ] [Review][Patch] R3-H-S4-3 (High) Missing Vitest page test for `volunteers/page.tsx` (AC-9 mandate) (AA-9) [frontend/src/app/(dashboard)/events/[id]/volunteers/__tests__/page.test.tsx (new file)]
- [ ] [Review][Patch] R3-M-S4-1 (Medium) Reminder HTML body missing `<meta charset>` and `<html lang>` (BH-11) [backend/src/IabConnect.Infrastructure/Events/EventNotificationService.cs]
- [ ] [Review][Patch] R3-M-S4-2 (Medium) Timezone fallback silent (LogWarning + UTC) — promote to LogError + health check (BH-14) [backend/src/IabConnect.Api/DependencyInjection.cs (ResolveReminderJobTimeZone)]
- [ ] [Review][Patch] R3-M-S4-3 (Medium) Cancellation mid-batch loses `LogInformation` summary — wrap in try/finally (EC-19) [backend/src/IabConnect.Application/Events/Jobs/VolunteerShiftReminderService.cs:55-119]
- [ ] [Review][Patch] R3-M-S4-5 (Medium) Reminder query lacks member-status filter (Inactive/merged members get reminded) (AA-15) [backend/src/IabConnect.Application/Events/Jobs/VolunteerShiftReminderService.cs:62, backend/src/IabConnect.Infrastructure/Persistence/Repositories/EventVolunteerAssignmentRepository.cs (GetRemindersDueAsync)]
