...
As an Admin,
I want to merge duplicate members,
so that history and references remain intact.

## Acceptance Criteria

1. Given an Admin has the required permission and prerequisite data exists, when they merge duplicate members, then admin selects source and target member records.
2. Given the sensitive action succeeds or fails, when audit evidence is reviewed, then merge preserves important references, including consents, documents, event registrations, finance references, and audit records where applicable.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then unsafe merges are blocked with actionable reasons.
4. Given the sensitive action succeeds or fails, when audit evidence is reviewed, then merge writes audit evidence with source and target IDs.
5. Given a caller lacks the required permission or role, when the protected behavior is requested, then merge cannot be performed without member write/merge permission.
...
- Multi-epic sprint-plan order: 9.
...
- Hard deleting source records without preserving references
- Merging without audit evidence

### Existing Code To Inspect Before Editing

- MergeMembersCommand
- MemberMergeHistory if audit alone is insufficient
- MemberRepository.cs
- MemberEndpoints.cs
...
- Build on `e2-s2-show-duplicate-warnings-in-member-create-edit.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e2-s3-implement-safe-member-merge`.
