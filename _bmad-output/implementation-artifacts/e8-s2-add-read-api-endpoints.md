...
As a integration,
I want scoped read APIs,
so that approved systems can consume IAB Connect data.

## Acceptance Criteria

1. Given integration has the required permission and prerequisite data exists, when scoped read APIs, then initial read APIs expose only approved low-risk resources.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then responses avoid leaking sensitive fields by default.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then pagination, filtering, and rate limits are applied.
4. Given valid inputs and existing system constraints, when this story behavior is exercised, then aPI contracts are documented.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then access denied and rate-limit behavior are testable.
...
- Multi-epic sprint-plan order: 31.
...
- external API route group
- integration-safe DTOs
- reused Application queries
- pagination/filter/rate-limit tests
- API contract docs
...
- Build on `e8-s1-add-api-credentials-and-scopes.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e8-s2-add-read-api-endpoints`.
