...
As an Admin or user,
I want the app to support language expansion,
so that Hindi can be introduced incrementally without hardcoded strings.

## Acceptance Criteria

1. Given Admin or user has the required permission and prerequisite data exists, when the app to support language expansion, then existing DE/EN translation behavior remains stable.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then hindi message file structure can be introduced.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then missing keys fall back safely.
4. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then new/touched UI has no hardcoded user-facing text.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then language preference persists where existing localization infrastructure supports it.
...
- Multi-epic sprint-plan order: 28.
- Requirement(s): REQ-055.
...
- Translating backend enum contract values
- Mass translating every page unless in scope

### Existing Code To Inspect Before Editing

- frontend/messages/hi.json if approved
- frontend/src/i18n configuration
- fallback handling
- message validation/typecheck
...
- Build on `e7-s2-improve-shared-component-accessibility.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.

- Recent git context before this planning pass: `feat(REQ-017): Segmentierung & Verteiler - vollstÃ¤ndige Implementierung`; reuse established member segment, audit, and communication patterns where relevant.
...
- Create-story validation completed on 2026-05-12 for `e7-s3-add-hindi-translation-expansion-path`.
