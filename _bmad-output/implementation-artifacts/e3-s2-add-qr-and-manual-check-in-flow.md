...
As a user,
I want Add QR and Manual Check-in Flow,
so that the product goal is satisfied.

## Acceptance Criteria

1. Given user has the required permission and prerequisite data exists, when Add QR and Manual Check-in Flow, then staff can scan a registration QR code and update check-in status.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then duplicate scans show current state and do not create duplicate attendance records.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then staff can manually search and check in an attendee.
4. Given the sensitive action succeeds or fails, when audit evidence is reviewed, then check-in changes are auditable.
5. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then UI handles camera unavailable and invalid QR states.
...
- Multi-epic sprint-plan order: 11.
...
- Roster export design changes beyond what scanner needs

### Existing Code To Inspect Before Editing

- CheckInRegistrationCommand
- ManualCheckInRegistrationCommand
- EventRegistrationEndpoints check-in routes
- event check-in scanner/manual lookup UI
...
- Build on `e3-s1-add-event-check-in-roster-and-export.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e3-s2-add-qr-and-manual-check-in-flow`.
