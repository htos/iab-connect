...
Status: in-progress (Round-3 review complete 2026-05-14 — 0 Critical + 4 High + 3 Medium + 2 Decisions resolved (DN-3: move audit to handler; DN-5: keep defensive Guid.Empty + sync AC) routed back to dev; 2 Defer logged)
...
3. **`EventRegistration.CheckIn` is refactored to be idempotent and returns a result.** Today [EventRegistration.cs:312-313](backend/src/IabConnect.Domain/Events/EventRegistration.cs#L312-L313) throws `InvalidOperationException("Participant is already checked in")` on a duplicate scan. **Refactor the entity** to:
    ...
    - On already-checked-in: return `new CheckInResult(WasAlreadyCheckedIn: true, CheckedInAt: this.CheckedInAt!.Value, CheckedInBy: this.CheckedInBy!.Value)` instead of throwing. **Do NOT mutate state or `UpdatedAt`.** This is the core idempotency contract per AC-2 of the epic ("duplicate scans show current state and do NOT create duplicate attendance records").
    - Keep the existing `Cancelled` and `Waitlisted` pre-checks (lines 308-311) as `InvalidOperationException` — these are domain-rule violations, not duplicate scans, and the command handler maps them to a typed `CheckInResultDto.ConflictReason` (see AC-3) without `LogAccessGranted`.
...
- [ ] [Review][Decision] R3-DN-3 CheckIn audit at endpoint vs handler — keep current (endpoint MapCheckInResult logs once) / move to handler / both (AA-6)
- [ ] [Review][Decision] R3-DN-5 `CheckedInBy ?? Guid.Empty` on idempotent path — keep defensive substitute / revert to fail-fast / sentinel-per-env (AA-10)

### Patches

- [ ] [Review][Patch] R3-H-S2-1 (High) QR check-in token: add `MaximumLength(64)` validator + DB index on `EventRegistrations.QrCodeToken` (EC-4) [backend/src/IabConnect.Application/Events/CheckIn/CheckInRegistrationCommandValidator.cs:19 + new migration]
- [ ] [Review][Patch] R3-H-S2-2 (High) `EventRegistration.CheckIn` idempotent guard misses `Status == CheckedIn` — add explicit short-circuit (EC-8) [backend/src/IabConnect.Domain/Events/EventRegistration.cs:308-344]
- [ ] [Review][Patch] R3-H-S2-4 (High) `EventRegistration.CreateForMember` accepts `Guid.Empty` for `memberId` — reject in factory + drop `?? Guid.Empty` at endpoint (EC-12) [backend/src/IabConnect.Domain/Events/EventRegistration.cs:131-160, backend/src/IabConnect.Api/Endpoints/EventRegistrationEndpoints.cs:350-358]
- [ ] [Review][Patch] R3-M-S2-1 (Medium) `CheckInByQrCodeAsync` opens transaction then early-outs on null lock — restructure to commit/dispose explicitly (BH-12 + EC-15) [backend/src/IabConnect.Infrastructure/Events/EventRegistrationCheckInService.cs:60-78]
- [ ] [Review][Patch] R3-M-S2-3 (Medium) `CheckInSearchHasher` uses Base64 (not Base64Url) — replace `+`/`/` (BH-22 + AA-13) [backend/src/IabConnect.Application/Events/CheckIn/CheckInSearchHasher.cs]
