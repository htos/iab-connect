...
**Autonomous-mode escape (A41/A43):** user directive verbatim — _"implementiere das ganze epic 30 mit allen stories ohne stopp. erst danach ein review und retro"_ — pre-declares no-stop autonomous mode; every DEC carries a story-recommended option → A32 step (d) skipped, resolutions below.

- **DEC-1 — decomposition depth.** (a) **Option B** — `dashboard-content` composition root + `kpi-card` extracted; the landing / role-badge / quick-actions / KPI sections stay **inline** in the root's `DashboardBody`. (b) Rationale: the story marks A recommended but explicitly permits B "if A risks copy drift"; byte-identity is the paramount constraint here (the net is the oracle), and inline blocks keep every variable (`isAdmin`/`isVorstand`/`settings.modules`/`t`/`tDash`/`kpiData`) in scope with **zero prop-threading risk** — extracting QuickActions/RoleBadge would require threading ~6 props correctly, a behaviour-change surface the net only partially covers. KpiCard was already a standalone module-scope component → a clean, drift-free extraction. (c) Consequence: AC-7 slice files = `api/dashboard-api.ts` + `types/dashboard.types.ts` + `hooks/use-dashboard-overview.ts` + `components/{dashboard-content,kpi-card}.tsx`; landing/role/quick-actions/KPI JSX co-located in `DashboardBody`.
- **DEC-2 — KPI query semantics.** (a) **Option A** — `useQuery` with `enabled: isAuthenticated && canViewKpis`, `retry: false`. (b) Rationale: story rec A; user autonomous; `enabled` = the god-page's FETCH gate (A97 — a non-privileged direct-nav reaches the no-KPI surface, not a stuck spinner); `retry: false` matches the god-page's single ungated fetch (A99 — the provider default `retry:1` would double-fetch + delay). (c) Consequence: the dashboard is on the program's server-state standard; the self-wrapped feature-local QueryClient ALSO sets `retry: false` (A103 precedent).
- **DEC-3 — landing/spinner branch placement.** (a) **Option A** — all 3 branches co-located in `DashboardBody` (the root owns auth-state branching, like the god-page). (b) Rationale: story rec A; user autonomous; the 3-way branch reads cleaner co-located. (c) Consequence: spinner + gradient-landing keep their bespoke full-screen frames; only the authenticated branch adopts `<PageShell>` (AC-8).

**Verbatim-relocation technique (byte-identity safety):** rather than hand-transcribe the 560-line authenticated JSX (copy-drift risk), `app/page.tsx` was `cp`'d to `dashboard-content.tsx` and transformed with surgical edits ONLY: (1) header (imports + `HomePage`→`DashboardContent` self-wrap + `DashboardBody` + hook-derived `kpiData`/`kpiLoading`/`kpiError` locals so the KPI JSX stays byte-identical); (2) the authenticated `<main className="min-h-[calc(100vh-4rem)] bg-gray-50 p-4 md:p-8"><div className="mx-auto max-w-7xl">` frame → `<PageShell>` (byte-identical DOM); (3) bottom DTOs + `KpiCard` removed (now in `types/` + `kpi-card.tsx`); then `prettier --write` (new file) fixed the frame-swap indentation (DOM unaffected). The unused `roles` destructure was dropped (behaviour-neutral — the god-page never used it; required so the new `--max-warnings=0`-gated file lints clean; the `t("roles.*")` calls are i18n keys, not the variable).

### Completion Notes List

- **Net-first to FULL coverage (A87/A103):** extended the E30-S3 seed `app/page.dashboard.test.tsx` (4 → **22 tests**) and confirmed **GREEN AT HEAD** before extraction — pins the 3-way branch + no-redirect invariant, the 4 landing links, all 3 role badges, **every module gate** (REQ-087 `!== false`, parametrized hide+show per `events/documents/members/communication/finance/partners` + the Admin no-module gate), the KPI success path (fetch URL `/api/v1/reports/dashboard` + `formatCHF` output via an identity normalizer for the de-CH U+202F narrow-no-break-space), the KPI error COPY (A79), and `canViewKpis=false → no fetch` (A97). After the migration the net survived with **ZERO spec edits** (the A103 transport-agnostic-oracle proof — the god-page→slice flip is invisible to the oracle); the pre-existing `page.test.tsx` REQ-086 branding anchor also stayed green unmodified.
- **BUILD-on-useApiClient migration (A88/A94):** no transport rewrite (the dashboard already used `useApiClient`). `api/dashboard-api.ts` = `DASHBOARD_OVERVIEW_URL` + `dashboardKeys` + `fetchDashboardOverview` (builders + keys only); `hooks/use-dashboard-overview.ts` owns the `useApiClient` call inside `useQuery` (`enabled` fetch-gate A97, `retry:false` A99, error re-thrown as `Error.message` so the surfaced COPY equals the god-page's `res.error` string — A79). `DashboardContent` self-wraps its own `QueryClientProvider({ staleTime:60s, retry:false })` (A103 / E27-S3 precedent) so the net renders the entry directly; nested under the app provider in production (nearest wins).
- **PageShell adoption (AC-8):** the authenticated dashboard frame is now `<PageShell>` (default `maxWidth="7xl"`) → byte-identical `<main className="min-h-[calc(100vh-4rem)] bg-gray-50 p-4 md:p-8"><div className="mx-auto max-w-7xl">…`. The spinner + gradient-landing branches keep their bespoke full-screen frames (not the standard content frame). The bespoke welcome header (`<div className="mb-8">` h1+p, NOT the flex PageHeader block) stays inline — PageHeader would change the DOM, so it is correctly NOT adopted.
- **Behaviour preserved (AC-1..6):** no redirect (the root has no `next/navigation` import — verified); the auth/role gates (`canViewKpis = isVorstand || isAdmin`; Finance KPI requires `canReadFinance`; member/comm/finance/partner quick-actions require `isVorstand||isAdmin`; Admin requires `isAdmin`); the module gates (`!== false` default-on, NOT flipped to truthy); the KPI cards + `colorClasses` map + `formatCHF`; the error/empty copy; the `home.*`/`common.*`/`roles.*`/`publicNav.*` keys; `hi.json` untouched. No route/contract/i18n change.
- **AC-Subitem completion (A29):** AC-1 ✅ (no-redirect 3-branch) · AC-2 ✅ (auth+role gating) · AC-3 ✅ (module gating `!== false`, net-pinned per module) · AC-4 ✅ (KPI data path + formatCHF + colorClasses + error copy A79) · AC-5 ✅ (i18n keys, en↔de, hi as-is) · AC-6 ✅ (no contract change; suite + next build green) · AC-7 ✅ (`features/dashboard` slice: api/types/hooks/components) · AC-8 ✅ (PageShell on the authenticated frame, byte-identical; no raw `/api/v1/...` in a component — it's in `dashboard-api.ts`).
- **Gates:** `tsc --noEmit` clean (TanStack's discriminated-union `isError` narrows `error`); `npx eslint <changed> --max-warnings=0` clean; `npx prettier --write` (new slice files) + `--check` (thin `app/page.tsx`) clean; `npx vitest run` = **216 files / 2009 tests green** (1991 → 2009 = +18 net extension; net survived zero-edit + branding anchor green); `next build` exit 0 (root `/` is `ƒ` dynamic). LF on every file (A73). `git diff --stat app/page.tsx` = 6 insertions / 782 deletions (god-page → thin entry; body relocated to the slice). This RETIRES the last god-page of the frontend-refactoring program (A82 residual closed).

### File List

**New (features/dashboard slice):**
- `frontend/src/features/dashboard/types/dashboard.types.ts` (6 KPI DTOs)
- `frontend/src/features/dashboard/api/dashboard-api.ts` (URL + `dashboardKeys` + `fetchDashboardOverview`)
- `frontend/src/features/dashboard/hooks/use-dashboard-overview.ts` (TanStack `useQuery`, A97 enabled-gate + A99 retry:false)
- `frontend/src/features/dashboard/components/dashboard-content.tsx` (composition root: self-wrapped QueryClientProvider + `DashboardBody` 3-branch; authenticated frame on PageShell)
- `frontend/src/features/dashboard/components/kpi-card.tsx` (extracted `KpiCard` + `colorClasses`)

**Modified:**
- `frontend/src/app/page.tsx` (785-line god-page → thin server entry rendering `<DashboardContent />`)
- `frontend/src/app/page.dashboard.test.tsx` (E30-S3 seed net extended 4 → 22 tests for full coverage)

**Unchanged anchor (verified green, not edited):** `frontend/src/app/page.test.tsx` (REQ-086 hero branding).

## Change Log

- 2026-06-12: Story implemented + DoD green (Status → review). Migrated the 785-line root dashboard god-page (`app/page.tsx`) into `features/dashboard/` (api/types/hooks/components + thin entry) — the FINAL god-page migration of the frontend-refactoring program. KPI data on TanStack `useQuery` (BUILD on useApiClient, A97 enabled-gate + A99 retry:false, A79 error-copy preserved); authenticated frame on PageShell (byte-identical); `DashboardContent` self-wraps its QueryClientProvider (A103). Net extended 4→22 tests, GREEN at HEAD then survived the migration with ZERO spec edits. DEC-1=B (kpi-card extracted, blocks inline — byte-identity over decomposition), DEC-2=A (useQuery+retry:false), DEC-3=A (branches co-located). Vitest 1991→2009 (+18); tsc/eslint/prettier clean; next build exit 0; LF.
