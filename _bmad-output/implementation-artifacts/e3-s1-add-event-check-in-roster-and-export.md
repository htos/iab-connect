...
Status: in-progress (Round-3 review complete 2026-05-14 — 1 Critical + 1 High + 4 Medium + 2 Low + 1 Decision (DN-6: full English) routed back to dev; see Round 3 Review Findings section)
...
5. **CSV format is event-day-print-friendly.** CSV MUST use UTF-8 with a BOM (so Excel on Windows opens it correctly with Umlauts), CRLF line endings, comma separator, and RFC-4180-style quoting (fields containing `,`, `"`, CR, LF MUST be enclosed in `"…"`; embedded `"` doubled). Column order MUST be: `#` (1-based row index), `Name`, `Guests`, `Status`, `Waitlisted`, `CheckedIn`, `CheckedInAt` (ISO-8601 UTC, blank when null), `SpecialRequirements`, `QrCodeToken`, `[ ] (Anwesenheit)`. The last column MUST be an empty cell so staff can hand-check it on paper. File name MUST be `Checkin_<sanitizedEventTitle>_<YYYY-MM-DD>.csv` using the same `Path.GetInvalidFileNameChars()` sanitization as [EventRegistrationEndpoints.cs:576-577](backend/src/IabConnect.Api/Endpoints/EventRegistrationEndpoints.cs#L576-L577).
...
- [ ] [Review][Decision] R3-DN-6 CSV roster i18n: full German / full English / accept mix (BH-21, L-S1-2)

### Patches

- [ ] [Review][Patch] R3-C1 (Critical) CSV formula injection — sanitize leading `=`/`+`/`-`/`@` in `ParticipantName`/`SpecialRequirements` (EC-1) [backend/src/IabConnect.Infrastructure/Events/EventCheckInRosterCsvExporter.cs:105-128]
- [ ] [Review][Patch] R3-H-S1-1 (High) `ExportEventCheckInRosterQueryHandler` re-enters MediatR — inject the read handler directly (BH-3) [backend/src/IabConnect.Application/Events/CheckIn/ExportEventCheckInRosterQueryHandler.cs]
- [ ] [Review][Patch] R3-M-S1-1 (Medium) Excel-collapse risk on trailing empty tick-box column — emit quoted-empty for last cell (BH-18) [backend/src/IabConnect.Infrastructure/Events/EventCheckInRosterCsvExporter.cs (QuoteIfNeeded)]
- [ ] [Review][Patch] R3-M-S1-2 (Medium) Roster handler uses `DateTime.UtcNow`; inject `TimeProvider` (BH-15) [backend/src/IabConnect.Application/Events/CheckIn/GetEventCheckInRosterQueryHandler.cs]
- [ ] [Review][Patch] R3-M-S1-3 (Medium) `OrderBy(StringComparer.Ordinal)` may misorder Unicode names; use invariant-culture comparer (AA-11) [backend/src/IabConnect.Application/Events/CheckIn/GetEventCheckInRosterQueryHandler.cs:67]
- [ ] [Review][Patch] R3-M-S1-4 (Medium) AC-5 text says 10 columns; code emits 9 per D-S1-1 — sync AC text (AA-2) [`_bmad-output/implementation-artifacts/e3-s1-add-event-check-in-roster-and-export.md` AC-5]
- [ ] [Review][Patch] R3-L-S1-1 (Low) `[ ]` literal in CSV header may confuse Excel name-range parsing (EC-24) [backend/src/IabConnect.Infrastructure/Events/EventCheckInRosterCsvExporter.cs:31]
