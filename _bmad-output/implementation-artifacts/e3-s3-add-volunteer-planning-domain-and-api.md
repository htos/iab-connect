...
Status: in-progress (Round-3 review complete 2026-05-14 — 2 Critical IDORs (C2 UpdateRole + C3 GetAssignments) + 6 High + 3 Medium + 1 Decision resolved (DN-4: new `RequireEventStaffOrMember` policy) routed back to dev; 2 Defer logged)
...
- [ ] [Review][Decision] R3-DN-4 `event-manager` excluded from `RequireMember` policy — add role to policy / new `RequireEventStaffOrMember` policy / Keycloak composite role (AA-8)

### Patches

- [ ] [Review][Patch] R3-C2 (Critical) Cross-event IDOR: `PUT /volunteer-roles/{roleId}` ignores `eventId` (EC-2) [backend/src/IabConnect.Application/Events/Volunteers/Commands/UpdateEventVolunteerRoleCommand.cs:7-11, backend/src/IabConnect.Api/Endpoints/EventVolunteerEndpoints.cs:170-195]
- [ ] [Review][Patch] R3-C3 (Critical) Cross-event IDOR: `GET /volunteer-shifts/{shiftId}/assignments` ignores `eventId` (EC-3) [backend/src/IabConnect.Application/Events/Volunteers/Queries/GetVolunteerShiftAssignmentsQuery.cs:7, backend/src/IabConnect.Api/Endpoints/EventVolunteerEndpoints.cs:280-285]
- [ ] [Review][Patch] R3-H-S3-1 (High) `CancelShift` endpoint: cross-event mismatch returns 404 with no `LogAccessDenied` — distinguish "wrong event" outcome (BH-4) [backend/src/IabConnect.Api/Endpoints/EventVolunteerEndpoints.cs, backend/src/IabConnect.Infrastructure/Events/Volunteers/EventVolunteerAssignmentService.cs (CancelAllAssignmentsForShiftAsync)]
- [ ] [Review][Patch] R3-H-S3-2 (High) `EventVolunteerRoleRepository.GetByEventAndNameAsync` Turkish-locale `ToLower()` mismatch — use `EF.Functions.ILike` or `ToLowerInvariant` (BH-5) [backend/src/IabConnect.Infrastructure/Persistence/Repositories/EventVolunteerRoleRepository.cs]
- [ ] [Review][Patch] R3-H-S3-3 (High) `AddAtomicAsync` rethrows raw `DbUpdateException` on null re-fetch — return typed `Cancelled`/`Transient` outcome (BH-7) [backend/src/IabConnect.Infrastructure/Persistence/Repositories/EventVolunteerAssignmentRepository.cs]
- [ ] [Review][Patch] R3-H-S3-4 (High) Cancel-by-merged-member path returns NotAuthorized — follow `MergedIntoMemberId` once (BH-10) [backend/src/IabConnect.Application/Events/Volunteers/Commands/CancelVolunteerAssignmentCommand.cs]
- [ ] [Review][Patch] R3-H-S3-5 (High) N+1: `GetEventVolunteerShifts` runs 2N+2 queries — single GROUP BY (EC-9) [backend/src/IabConnect.Application/Events/Volunteers/Queries/GetEventVolunteerShiftsQuery.cs:33-41]
- [ ] [Review][Patch] R3-H-S3-6 (High) N+1: `GetVolunteerShiftAssignmentsQueryHandler` loads members one-by-one — add `IMemberRepository.GetByIdsAsync` batch (EC-10) [backend/src/IabConnect.Application/Events/Volunteers/Queries/GetVolunteerShiftAssignmentsQuery.cs:34-40]
- [ ] [Review][Patch] R3-M-S3-1 (Medium) `UpdateEventVolunteerShiftCommand` fields updated outside capacity transaction (TOCTOU) (EC-17) [backend/src/IabConnect.Application/Events/Volunteers/Commands/UpdateEventVolunteerShiftCommand.cs:84-112]
- [ ] [Review][Patch] R3-M-S3-2 (Medium) `CancelAllAssignmentsForShiftAsync` lost-update vs concurrent `CancelAssignmentAsync` — add `xmin` concurrency token (EC-23) [backend/src/IabConnect.Infrastructure/Events/Volunteers/EventVolunteerAssignmentService.cs:256-267]
- [ ] [Review][Patch] R3-M-S3-3 (Medium) `SelfSignUpForVolunteerShiftCommandHandler` throws `InvalidOperationException` mapped to 403 via string-message — add `NoMemberLink` typed outcome (BH-20) [backend/src/IabConnect.Application/Events/Volunteers/Commands/SelfSignUpForVolunteerShiftCommand.cs, backend/src/IabConnect.Api/Endpoints/EventVolunteerEndpoints.cs]
