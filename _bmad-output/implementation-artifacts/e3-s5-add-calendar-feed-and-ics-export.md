...
Status: review (Round-3 fix-pass complete 2026-05-14 — 2 Decisions + 4 High + 5 Medium + 1 Low resolved; backend tests 1776 / 1776 green; 5 items deferred to follow-up tickets with rationale (HMAC pepper migration, rotate row-lock, two missing test files, Visibility×Status matrix centralisation, line-folding post-escape budget). The DN-1 decision was implemented as option (c) — document rollback as backup-only — rather than the more invasive option (b) additive refactor; DN-2 was implemented as option (a) — broaden the filter to include Pending alongside Active.)
