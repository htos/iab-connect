...
I want calendar feeds or event exports,
so that I can subscribe to association events.

## Acceptance Criteria

1. Given member or public visitor has the required permission and prerequisite data exists, when calendar feeds or event exports, then public iCal feed includes only public events.
2. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then per-event `.ics` export includes stable UID, title, date/time, location, and summary.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then member-only/private events are not exposed through unauthenticated public feeds.
4. Given valid inputs and existing system constraints, when this story behavior is exercised, then event updates preserve stable calendar identity.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then public website can link or embed calendar output safely.
...
- Multi-epic sprint-plan order: 14.
- Requirement(s): REQ-025.
...
- Google Calendar API write integration
- Exposing private/member-only events anonymously

### Existing Code To Inspect Before Editing

- ICS generator
- public/member calendar endpoints
- frontend/public event links
...
- Build on `e3-s4-add-volunteer-planning-ui-and-reminders.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e3-s5-add-calendar-feed-and-ics-export`.
