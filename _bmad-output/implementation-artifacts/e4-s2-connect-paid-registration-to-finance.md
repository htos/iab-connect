...
As a Kassier,
I want paid event registration to create finance records,
so that fees can be tracked and reconciled.

## Acceptance Criteria

1. Given a Kassier has the required permission and prerequisite data exists, when paid event registration to create finance records, then paid registration creates the required invoice, payment request, receipt, or finance record according to finance rules.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then finance record references the event registration.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then cancellation/refund uses cancellation/reversal behavior, not hard delete.
4. Given a caller lacks the required permission or role, when the protected behavior is requested, then finance permissions are required for finance approval/reconciliation actions.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then failure to create finance records does not leave inconsistent registration state.
...
- Multi-epic sprint-plan order: 16.
- Requirement(s): REQ-022.
- Epic goal: Support paid event registration through existing Events and Finance modules.
...
- Hard deleting paid registrations or finance records

### Existing Code To Inspect Before Editing

- Application coordination service between Events and Finance
- registration-to-finance transaction behavior
- invoice/payment/receipt reference from event registration
...
- Paid registration crosses Events and Finance; use an Application service and transaction where state must be atomic.
- Do not bypass finance cancellation, reversal, soft-delete, sent-invoice, overdue, or posted-entry rules.
- Money display must not rely on hardcoded formatting assumptions.
...
- Build on `e4-s1-add-event-fee-configuration.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e4-s2-connect-paid-registration-to-finance`.
