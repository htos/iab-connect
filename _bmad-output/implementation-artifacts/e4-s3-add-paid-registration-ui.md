...
I want to understand event fees during registration,
so that I can confirm a paid event registration.

## Acceptance Criteria

1. Given member or public visitor has the required permission and prerequisite data exists, when they understand event fees during registration, then registration page shows fee category and amount.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then confirmation email includes ticket or payment information.
3. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then UI handles free, paid, waitlisted, cancelled, and payment-pending states.
4. Given a caller lacks the required permission or role, when the protected behavior is requested, then admin/event manager participant view shows fee/payment state.
5. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then UI text uses translations and existing event components.
...
- Multi-epic sprint-plan order: 17.
...
- Backend finance orchestration beyond gaps from E4-S2

### Existing Code To Inspect Before Editing

- public/member registration fee display
- admin participant payment state display
- typed event/finance DTO usage
...
- Build on `e4-s2-connect-paid-registration-to-finance.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e4-s3-add-paid-registration-ui`.
