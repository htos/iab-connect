...
| 2026-05-15 | Round-2 epic-boundary re-review: 1 ex-decision + 3 patches applied (1 patch demoted to defer). DN-1 → `hasAuthSession` now `await getToken({ req, secret })` (next-auth v4 JWT validation, replaces cookie-name regex — forged cookies no longer bypass the neutral-UX rewrite). `resolveModuleForPath` strips trailing slashes. `sanitizeModuleMap` adds `Object.hasOwn` + `__proto__` guard. `AppSettingsProvider` warns when `data.modules` is missing/non-object on a 2xx response. `middleware.test.ts` reshaped: 5-case cookie-shape `it.each` replaced with positive/negative `vi.doMock("next-auth/jwt")` tests. P-S4-4 (Sidebar submenu test) demoted to defer (untestable without Sidebar refactor). 2 defers added (empty-submenu drop, combined-filter test coverage). Frontend Vitest 89/89, typecheck + lint green. Status → done. |
...
_E10-S4 routing: 1 decision, 4 patches, 2 defers._

- [x] [Review][Patch] (was DN-1, Decision → Patch 2026-05-15) `hasAuthSession` matched only cookie name-shapes, not the JWT — a forged `authjs.session-token=anything` cookie defeated the `public_view=false` rewrite. **Chosen direction:** call next-auth v4 `getToken({ req, secret })` in middleware [frontend/src/middleware.ts:97-103] — **RESOLVED 2026-05-15:** `hasAuthSession` is now async, awaits `getToken({ req: request, secret: process.env.NEXTAUTH_SECRET })` and returns `token !== null`. The HMAC signature is verified, so a forged cookie returns null. `middleware.test.ts` re-shaped: the cookie-shape `it.each` (5 cases) replaced with one positive (authenticated → no rewrite) and one negative (forged cookie value → rewrite) using `vi.doMock("next-auth/jwt")`. Cookie variant handling (chunked / `__Secure-` / `__Host-` / Auth.js v5 names) is now next-auth/jwt's responsibility.
- [x] [Review][Patch] `frontend/src/lib/modules.ts::resolveModuleForPath` did not normalize trailing slashes — `/finance/` slipped through the guard [frontend/src/lib/modules.ts] — **RESOLVED 2026-05-15:** path is normalized via `pathname.replace(/\/+$/, "")` before prefix comparison; root `/` is preserved.
- [x] [Review][Patch] `frontend/src/lib/modules.ts::sanitizeModuleMap` iterated `Object.entries(raw)` — an own `__proto__` / `constructor` key from `JSON.parse` would survive [frontend/src/lib/modules.ts] — **RESOLVED 2026-05-15:** added `Object.hasOwn(raw, key) && key !== "__proto__"` guard before assigning into the result; prototype-pollution-style payloads now drop silently.
- [x] [Review][Defer] `Sidebar.test.tsx` does not exercise the submenu pre-filter (P-S4-4 demoted from Patch → Defer on 2026-05-15) — the inline arrow `isModuleEnabled` filter cannot be tested without exporting/refactoring `navItems` from `Sidebar.tsx`; the branch is dead in current shipped data (no submenu items carry `requiresModule`) [frontend/src/components/navigation/Sidebar.test.tsx] — deferred, combined-filter coverage track owns this alongside the role+module+double-entry defer below
- [x] [Review][Patch] `AppSettingsProvider.fetchSettings` accepted `data.modules === undefined` on a 2xx response and silently defaulted to all-modules-enabled [frontend/src/components/providers/AppSettingsProvider.tsx] — **RESOLVED 2026-05-15:** added `console.warn` when `data.modules` is missing / null / non-object / array on a successful fetch — a backend regression that drops the projection now surfaces in devtools.
- [x] [Review][Defer] `Sidebar` parent `NavItem` whose `submenu.filter(isModuleEnabled)` leaves the submenu empty renders as an empty expandable group — dead branch today (no submenu items carry `requiresModule`); defensively drop empty parents when this surface activates [frontend/src/components/navigation/Sidebar.tsx] — deferred, defensive future-proofing
- [x] [Review][Defer] `Sidebar.test.tsx` mocks `useApiClient` to error every GET to silence the finance-profile fetch — combined-filter coverage (role + `requiresModule` + `requiresDoubleEntry`) is not asserted [frontend/src/components/navigation/Sidebar.test.tsx] — deferred, test-coverage gap; not blocking
