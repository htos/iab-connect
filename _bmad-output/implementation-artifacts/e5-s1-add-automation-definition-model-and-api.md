...
I want to define automation journeys,
so that standard messages can be triggered automatically.

## Acceptance Criteria

1. Given a Communication user has the required permission and prerequisite data exists, when they define automation journeys, then authorized user can create, edit, pause, resume, and disable automation definitions.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then automation definitions include trigger, template, recipient rules, and status.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then recipient rules respect consent and role/member filters.
4. Given valid inputs and existing system constraints, when this story behavior is exercised, then invalid automation definitions are rejected by validators.
5. Given the sensitive action succeeds or fails, when audit evidence is reviewed, then changes are auditable.
...
- Multi-epic sprint-plan order: 18.
...
- Executing sends
- Provider-specific SMS/WhatsApp

### Existing Code To Inspect Before Editing

- AutomationDefinition/AutomationTrigger
- automation commands/queries/validators
...
- Create-story validation completed on 2026-05-12 for `e5-s1-add-automation-definition-model-and-api`.
