...
As a Kassier,
I want to create cost centers and budgets,
so that event/project costs can be planned and tracked.

## Acceptance Criteria

1. Given a Kassier has the required permission and prerequisite data exists, when they create cost centers and budgets, then kassier/Vorstand can create, edit, and deactivate cost centers.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then budget values can be assigned by fiscal period.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then validation prevents invalid periods, duplicate active names where inappropriate, and negative budget values unless explicitly allowed.
4. Given a caller lacks the required permission or role, when the protected behavior is requested, then changes respect finance permissions.
5. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then eF migration adds required tables/configuration.
...
- Multi-epic sprint-plan order: 23.
...
- Assigning records to cost centers
- Budget reports

### Existing Code To Inspect Before Editing

- CostCenter/CostCenterBudget
- Finance commands/queries/validators
...
- Create-story validation completed on 2026-05-12 for `e6-s1-add-cost-center-and-budget-model`.
