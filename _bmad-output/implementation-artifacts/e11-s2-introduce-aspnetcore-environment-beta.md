...
Adversarial review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) over the full Epic E11 diff (~2772 lines, S1+S2+S3 bundled, d55a57c..HEAD + working tree). ~38 raw findings → 4 decision-needed, 6 patch, 7 defer, ~17 dismissed. Findings affecting E11-S2 listed here; E11-S3 parser patches live in that story's Review Findings.

**Decisions resolved (2026-05-16):** D1 and D2 → defer (rustfsadmin: accepted Follow-up vor Beta-Deploy / pre-Beta IOptions refactor; AC-9 test gap: defer auf E15-S2 unblocker). D3 and D4 → patch (case-insensitive compare; orphan-job RemoveIfExists).

**Patch (6) — unambiguous fixes ready to apply (E11-S2 scope):**

- [x] [Review][Patch] **(from D3)** BetaBanner case-sensitive `envLabel === "beta"` excludes `Beta` (matching ASPNETCORE_ENVIRONMENT casing) [frontend/src/components/navigation/BetaBanner.tsx:27] — replace strict-compare with `envLabel?.toLowerCase().trim() === "beta"` so `NEXT_PUBLIC_ENV_LABEL=Beta` / `BETA` / `beta` all render. Add 2-3 test cases in BetaBanner.test.tsx.
- [x] [Review][Patch] **(from D4)** Orphaned Hangfire `enforce-retention-policies` schedule survives flag flip `true→false` [backend/src/IabConnect.Api/DependencyInjection.cs:372-382] — add `else { jobManager.RemoveIfExists(RetentionJobId); }` to `RegisterRetentionEnforcementJob` so a Beta deployment that previously ran with the E11-S1 baseline (flag defaults to true, schedule registered) has the orphan removed on flag-flip. ADR-020 prevents Beta tester data deletion only if the schedule is actually gone. Add 1 test case to `RetentionEnforcementJobRegistrationTests` asserting `RemoveIfExists` invocation when flag is false.
- [x] [Review][Patch] BetaBanner sessionStorage exceptions surface as silent console errors [frontend/src/components/navigation/BetaBanner.tsx:42-54] — wrap `sessionStorage.getItem`/`setItem` calls in try/catch with a one-line warn; Safari Private Browsing throws `QuotaExceededError` on `setItem`. ~5 lines.
- [x] [Review][Patch] BetaBanner flash-of-orange-banner for dismissed users [frontend/src/components/navigation/BetaBanner.tsx:42-54] — replace `useState(true) + useEffect` with lazy initializer `useState(() => typeof window === "undefined" || sessionStorage.getItem(DISMISSED_KEY) !== "1")`. SSR-safe (the component is `"use client"` so it never SSRs) and avoids the one-render orange flash before the effect ticks.
- [x] [Review][Patch] `NEXT_PUBLIC_FEEDBACK_URL` is consumed in code but undocumented in `frontend/.env.example` [frontend/src/components/navigation/BetaBanner.tsx:56-58, frontend/.env.example] — story Dev Notes promised the append (E11-S2 §"Why feedback URL has a placeholder default") but the diff doesn't include it. One-line entry under the existing `# === Beta + OSS build-time vars ===` section.
- [x] [Review][Patch] BetaBanner feedback URL fallback hardcodes upstream `htos/iab-connect` repo (not fork-friendly) [frontend/src/components/navigation/BetaBanner.tsx:57-59] — pattern-match the existing `NEXT_PUBLIC_SOURCE_URL` env-var: `process.env.NEXT_PUBLIC_FEEDBACK_URL ?? (process.env.NEXT_PUBLIC_SOURCE_URL ?? "https://github.com/htos/iab-connect") + "/issues/new?template=beta-feedback.md"`. Forks set `NEXT_PUBLIC_SOURCE_URL` and the feedback link redirects with no extra config.
...
**Defer (7) — real but not actionable now, logged in `deferred-work.md`:**

- [x] [Review][Defer] **(from D1)** AC-2 partial leaves committed `rustfsadmin` dev credentials in base `appsettings.json` [backend/src/IabConnect.Api/appsettings.json:38-41] — defer accepted; pre-Beta-deploy follow-up story (IOptions refactor for DocumentStorage per spec's Fix-path #1, low risk mechanical) will close before any actual Beta deployment. Wave 3+/post-E15-S2 territory. Existing deferred-work entry "E11-S2 follow-up: eager-init keys block cleanup" covers the refactor work.
- [x] [Review][Defer] **(from D2)** AC-9 test coverage gap — `BetaEnvironmentHardeningTests` covers 1 of 5 specified hardenings [backend/tests/IabConnect.Api.Tests/BetaEnvironmentHardeningTests.cs] — defer accepted; blocked by E15-S2 `Database__AutoMigrate` which unblocks a real Beta-WebApplicationFactory. Until then, code-audit (Task 5) + manual smoke (Task 11) compensate. When E15-S2 lands, extend BetaEnvironmentHardeningTests to assert Swagger 404 / Hangfire 404 / HSTS / HTTPS-redirect / strict-CORS at the HTTP pipeline level.

- [x] [Review][Defer] BetaBanner test mutates `process.env.NEXT_PUBLIC_ENV_LABEL` at runtime to test build-time-inlined behavior [frontend/src/components/navigation/BetaBanner.test.tsx:30-50] — tests pass under Vitest (Node sees the env-var write) but do NOT prove Next.js production-build inlining. The R2 risk in the story Dev Notes is exactly this; no automated coverage. Defer because a real fix needs a Next.js build-output integration test that this project doesn't have infrastructure for yet.
- [x] [Review][Defer] `appsettings.Beta.json:7-12` `Logging.LogLevel.Default = "Information"` may duplicate `appsettings.json:6` `Serilog.MinimumLevel.Default = "Information"` [backend/src/IabConnect.Api/appsettings.Beta.json:7-12] — needs runtime check whether `Logging:LogLevel` is load-bearing when Serilog hosting is wired (it usually isn't). Defer for a focused 30-minute investigation; if non-load-bearing, remove the section.
- [x] [Review][Defer] Empty `Keycloak.Authority` / `Smtp.Host` in base produce slow runtime failures (JwtBearer IDX20803 on first authed request; SocketException after ~100s on first mail send) instead of fast startup-time configuration errors [backend/src/IabConnect.Api/DependencyInjection.cs:132, backend/src/IabConnect.Infrastructure/Email/SmtpEmailSender.cs:31] — same root-cause family as the E11-S1 follow-up "eager-init keys block cleanup" entry; the IOptions refactor + startup validation chain that closes D1 also closes this. Defer (covered by existing deferred-work entry).
- [x] [Review][Defer] `AppSettingsLayeringTests` excludes the 5 deferred eager-init keys (`ConnectionStrings:DefaultConnection`, 4 `DocumentStorage:*`) [backend/tests/IabConnect.Api.Tests/AppSettingsLayeringTests.cs] — Theory only asserts `Keycloak:Authority` + `Smtp:Host`. A future commit that re-introduces a localhost default for any of the 5 deferred keys would not regression-test. Defer (already disclosed in deferred-work.md; tied to D1 resolution).
- [x] [Review][Defer] E11-S2 Task 11 (manual smoke tests) marked `[x]` despite Completion Notes admitting smoke tests were NOT executed — process smell, not a code defect [E11-S2 story file Task 11 + Completion Notes] — pattern is repo-wide; address in epic retrospective process improvement, not as a code patch.

(Plus 17 findings dismissed as noise/INFO: case-only patterns, single-element array constraints, line-number rot, RetentionEnforcement string-"0" .NET binding, AppSettingsLayeringTests parent-walk, Hangfire MockBehavior.Strict brittleness, etc. — full triage transcript in conversation context, not persisted.)
...
| 2026-05-16 | Epic-E11 boundary code review + 6 patches applied. | Adversarial review over full E11 diff (S1+S2+S3, ~2772 lines) produced 4 decisions + 6 patches + 7 defers + ~17 dismisses. Decisions resolved: D1 + D2 → defer (rustfsadmin pre-Beta follow-up, AC-9 test gap blocked by E15-S2); D3 + D4 → patch (case-insensitive envLabel, orphan-job RemoveIfExists). 6 patches applied: BetaBanner case-insensitive `envLabel?.trim().toLowerCase() === "beta"`, BetaBanner sessionStorage try/catch (Safari Private Browsing safety), BetaBanner lazy useState initializer (no flash-of-orange for dismissed users), BetaBanner fork-friendly feedback URL via NEXT_PUBLIC_SOURCE_URL fallback, `NEXT_PUBLIC_FEEDBACK_URL` documented in frontend/.env.example, RegisterRetentionEnforcementJob `else { RemoveIfExists }` branch. Test deltas: backend +0 net (1 test renamed Register_DoesNothing → Register_RemovesJob, 1957/1957 stays green); frontend Vitest +14 (113 → 127 across 17 files: +7 parser hardening cases in E11-S3, +7 BetaBanner case-insensitive ×4 / fork-friendly / sessionStorage exception ×2). Smoke build green with all 3 Beta env vars baked. Status → done. |
