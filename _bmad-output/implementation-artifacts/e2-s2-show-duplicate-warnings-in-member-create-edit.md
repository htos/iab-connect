...
As an Admin,
I want warnings during member create/edit,
so that I can stop before saving a duplicate.

## Acceptance Criteria

1. Given an Admin has the required permission and prerequisite data exists, when warnings during member create/edit, then create and edit flows check for duplicate candidates.
2. Given a caller lacks the required permission or role, when the protected behavior is requested, then UI shows candidate summaries and links where authorized.
3. Given a caller lacks the required permission or role, when the protected behavior is requested, then admin can continue only through an explicit confirmation when policy allows it.
4. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then UI text uses next-intl.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then existing member validation remains intact.
...
- Multi-epic sprint-plan order: 7.
...
- Bulk duplicate review page
- Merge execution

### Existing Code To Inspect Before Editing

- frontend/src/app/members/new
- frontend/src/app/members/[id]/edit
...
- Build on `e2-s1-add-duplicate-candidate-detection.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e2-s2-show-duplicate-warnings-in-member-create-edit`.
