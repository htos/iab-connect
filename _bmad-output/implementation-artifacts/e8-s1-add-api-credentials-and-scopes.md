...
As an Admin/IT user,
I want to create scoped API credentials,
so that integrations can access only approved resources.

## Acceptance Criteria

1. Given an Admin/IT user has the required permission and prerequisite data exists, when they create scoped API credentials, then admin can create and revoke API credentials.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then credentials are shown only once and stored using a token-safe design.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then scopes limit accessible resources/actions.
4. Given the sensitive action succeeds or fails, when audit evidence is reviewed, then credential use is logged.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then external API routes enforce scopes and rate limits.
...
- Multi-epic sprint-plan order: 30.
...
- Read API endpoints beyond credential smoke tests

### Existing Code To Inspect Before Editing

- ApiClient/ApiScope
- token generation/hash verification service
- admin endpoints/UI for create/revoke
- rate-limit/scope plumbing
...
- Create-story validation completed on 2026-05-12 for `e8-s1-add-api-credentials-and-scopes`.
