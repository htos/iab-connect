...
As a Communication user,
I want a management UI for automations,
so that I can configure, preview, and monitor journeys.

## Acceptance Criteria

1. Given a Communication user has the required permission and prerequisite data exists, when a management UI for automations, then UI lists automations with status, trigger, template, and recent execution state.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then user can create/edit automation definitions.
3. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then user can preview recipients before activation.
4. Given valid inputs and existing system constraints, when this story behavior is exercised, then user can pause/resume/disable automations.
5. Given the page is loaded in desktop and mobile layouts, when the relevant state is displayed, then UI uses shared components, filters/search, and translated text.
...
- Multi-epic sprint-plan order: 20.
- Requirement(s): REQ-028.
...
- Execution engine rewrites unless E5-S2 gap exists

### Existing Code To Inspect Before Editing

- frontend/src/app/communication/automations
- automation API wrapper
- automation form/status/list components
- message keys
...
- Build on `e5-s2-add-automation-execution-engine.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e5-s3-add-automation-management-ui`.
