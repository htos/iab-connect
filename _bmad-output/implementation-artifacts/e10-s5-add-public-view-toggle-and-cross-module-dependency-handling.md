...
| 2026-05-15 | Round-2 epic-boundary re-review: 5 [Review][Patch] items applied. `SettingsEndpoints.GetPublicSettings` module fetch wrapped in `try/catch` + empty-map fallback (branding survives module-service outage). `ModuleEnforcementEndpointTests.AlwaysOnAnonymousEndpoints` extended with `/api/v1/events/my-calendar.ics` + `/api/v1/events/{guid}/calendar.ics`. `ModulePublicViewEndpointTests` adds `PublicRsvp_Returns403_WhenPublicViewDisabled` + `PublicContactSubmit_Returns403_WhenPublicViewDisabled`. `middleware.ts` /public + /public/unsubscribe explicit-segment matches (no more `/publication`, `/public/unsubscribe-page` collisions); new `/public/unsubscribe-page` regression test. `site-unavailable/page.tsx` `console.warn` on branding fetch failure. 1 defer added (RSVP defensive remap). Backend 1942/1942 green, frontend Vitest 89/89 green. Status → done. |
...
_E10-S5 routing: 5 patches, 1 defer._

- [x] [Review][Patch] `SettingsEndpoints.GetPublicSettings` now `await moduleSettingsService.GetAllAsync(ct)` — a module-service failure would 500 the anonymous public-settings endpoint and lose branding **and** the module map at once [backend/src/IabConnect.Api/Endpoints/SettingsEndpoints.cs] — **RESOLVED 2026-05-15:** module fetch wrapped in `try/catch` with empty-map fallback — branding always renders, missing module entries default to enabled on the frontend (behaviour-preserving + consistent with middleware degrade-to-enabled posture).
- [x] [Review][Patch] `ModuleEnforcementEndpointTests.AlwaysOnAnonymousEndpoints_StayReachable_WithModulesDisabled` covered `/api/v1/settings/public` and `/api/v1/events/calendar.ics` only [backend/tests/IabConnect.Api.Tests/Endpoints/ModuleEnforcementEndpointTests.cs] — **RESOLVED 2026-05-15:** test extended with `/api/v1/events/my-calendar.ics` and `/api/v1/events/{guid}/calendar.ics` assertions (using `NotBe(Forbidden)` since auth/token/ID may produce 401/404 — the proof is that the Module gate isn't intercepting).
- [x] [Review][Patch] `ModulePublicViewEndpointTests` `PublicEndpoints` theory covered blog / sponsor / events-public but not the public RSVP `POST /api/v1/events/{eventId}/registrations/public` nor the public contact `POST /api/v1/contact/...`. Auditor flagged as PARTIAL [backend/tests/IabConnect.Api.Tests/Endpoints/ModulePublicViewEndpointTests.cs] — **RESOLVED 2026-05-15:** two new facts — `PublicRsvp_Returns403_WhenPublicViewDisabled` and `PublicContactSubmit_Returns403_WhenPublicViewDisabled` — close the gap.
- [x] [Review][Patch] `frontend/src/middleware.ts` used `pathname.startsWith("/public/unsubscribe")` for the unsubscribe exemption and `pathname.startsWith("/public")` for the public_view gate — both broader than intended (e.g. `/public/unsubscribe-page`, `/publication`) [frontend/src/middleware.ts] — **RESOLVED 2026-05-15:** both matches are now explicit-segment: `pathname === "/public/unsubscribe" || pathname.startsWith("/public/unsubscribe/")` for the exemption, and `pathname === "/public" || pathname.startsWith("/public/")` for the public_view gate. New regression test `rewrites /public/unsubscribe-page (not an unsubscribe route)` pins the tightened behaviour.
- [x] [Review][Patch] `frontend/src/app/site-unavailable/page.tsx` swallowed all branding-fetch errors silently — operator with a misconfigured `NEXT_PUBLIC_API_URL` got no observability signal [frontend/src/app/site-unavailable/page.tsx] — **RESOLVED 2026-05-15:** `.catch` now calls `console.warn` with a clear message naming `NEXT_PUBLIC_API_URL` before falling back to the unbranded view. Existing Vitest `falls back to a plain unbranded message when the settings fetch fails` covers the path; warn is visible in test stderr.
- [x] [Review][Defer] Public RSVP endpoint mapped on the `Module:events`-gated parent group rather than a sibling un-gated group — currently safe because `.AllowAnonymous()` overrides the auth policy, but if `.AllowAnonymous` is removed in a future refactor the endpoint silently becomes `Module:events`-gated, breaking public RSVPs when events module is on but public_view is off. Defensively map on a separate group [backend/src/IabConnect.Api/Endpoints/EventRegistrationEndpoints.cs] — deferred, speculative future-regression
