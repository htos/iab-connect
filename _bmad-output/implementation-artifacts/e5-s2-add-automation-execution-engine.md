...
As a user,
I want Add Automation Execution Engine,
so that the product goal is satisfied.

## Acceptance Criteria

1. Given user has the required permission and prerequisite data exists, when Add Automation Execution Engine, then hangfire executes automation jobs.
2. Given valid inputs and existing system constraints, when this story behavior is exercised, then execution records track status, recipients, failures, and timestamps.
3. Given valid inputs and existing system constraints, when this story behavior is exercised, then idempotency prevents duplicate sends for the same trigger/recipient.
4. Given the sensitive action succeeds or fails, when audit evidence is reviewed, then failed sends are logged and visible.
5. Given valid inputs and existing system constraints, when this story behavior is exercised, then execution does not block unrelated user workflows.
...
- Multi-epic sprint-plan order: 19.
...
- Management UI forms beyond status visibility needed for testing

### Existing Code To Inspect Before Editing

- AutomationExecution/AutomationRecipient
...
- Build on `e5-s1-add-automation-definition-model-and-api.md` if present. Reuse its module paths, endpoint conventions, test style, and review findings.
...
- Create-story validation completed on 2026-05-12 for `e5-s2-add-automation-execution-engine`.
