...
- [x] **Task 0: Spike + resolve DECs** (AC: all) — A56 existing-implementation spike; record A43 (a)/(b)/(c) for each DEC in Debug Log.
  - [x] Grep `frontend/src` for `PageShell`/`PageHeader` → confirm the only hits are the two E28 `TODO(E30)` comments (no provisional component exists; AC-3 is a clean introduction).
  - [x] Confirm `frontend/src/components/layout/` does **not** exist yet and `components/ui` has no `index.ts` barrel (so the barrel convention to mirror is `components/navigation/index.ts`).
  - [x] Read [suppliers-page-content.tsx:76-152](../../frontend/src/features/suppliers/components/suppliers-page-content.tsx#L76-L152) to lock the exact frame + header markup the primitives must reproduce. Note the per-page `max-w-*` varies (suppliers `7xl`, module-unavailable `4xl`) → `maxWidth` prop with a **static class map** (DEC-2).
  - [x] Resolve DEC-1 (`<main>` vs `<div>` element), DEC-2 (maxWidth handling), DEC-3 (E28 residual resolution), DEC-4 (eslint leaf-boundary for `components/layout`).
- [x] **Task 1: `PageShell.tsx`** (AC: 6) — presentational, prop-driven, no `"use client"` (no hooks/state). Render `<main className="min-h-[calc(100vh-4rem)] bg-gray-50 p-4 md:p-8"><div className={\`mx-auto ${maxWidthClass}\`}>{header}{children}</div></main>`. `maxWidthClass` comes from a **static lookup object holding full class strings** (`{ "4xl": "max-w-4xl", "5xl": "max-w-5xl", "6xl": "max-w-6xl", "7xl": "max-w-7xl" }`, default `"7xl"`) — **never** template-interpolate `max-w-${maxWidth}` (Tailwind's JIT cannot see interpolated classes → the width silently disappears). Typed `PageShellProps` interface, no `any`.
- [x] **Task 2: `PageHeader.tsx`** (AC: 6) — presentational. Render the header block verbatim from the spike (title `<h1>`, optional `description` `<p>` rendered only when truthy, optional `actions` slot). Typed `PageHeaderProps`, no `any`. No `"use client"`. **Title/description are resolved strings passed in by the consumer** — `PageHeader` does NOT call `useTranslations` (the consuming page owns i18n; keeps the primitive pure + server-renderable).
- [x] **Task 3: Barrel `index.ts`** (AC: 5) — `export { PageShell } from "./PageShell"; export { PageHeader } from "./PageHeader";` plus `export type { PageShellProps } from "./PageShell"; export type { PageHeaderProps } from "./PageHeader";` if the prop types are useful to consumers. Mirror navigation/index.ts style.
- [x] **Task 4: Close the E28-S4 residual TODO markers** (AC: 7) — DEC-3. In [public-layout-shell.tsx:13-18](../../frontend/src/features/public/components/public-layout-shell.tsx#L13-L18) (and the matching note in `app/public/layout.tsx`), replace the open `TODO(E30): extract … into a slice-owned PageShell` promise with a **resolution comment**: PublicHeader/PublicFooter intentionally remain in `@/components/navigation/` (chrome, mirroring Header/Sidebar — moving header logic out is a hard-constraint violation); `PageShell` (the authenticated content-frame primitive introduced in E30-S1) does **not** apply to the public surface, whose shell is the distinct `public-layout-shell.tsx`. No code change to the public shell — comment-only. (These are pre-drifted files: comment-only edit, hand-match surrounding style, do NOT `prettier --write`, per A72/A81.)
- [x] **Task 5 (DEC-4): eslint leaf-boundary for `components/layout`** — add a fourth entry to `boundaryRules` in [eslint.config.mjs](../../frontend/eslint.config.mjs#L10-L66) mirroring the `components/ui` rule: `files: ["src/components/layout/**/*.{ts,tsx}"]`, `no-restricted-imports` forbidding `@/features`/`@/features/**`/`@/app`/`@/app/**` with a message "components/layout is a shared presentational layer and must not import from features or app (E21 boundary)." This keeps PageShell a true leaf (it must not reach into a feature). Features importing `@/components/layout` stays legal (no rule restricts the consumer direction).
- [x] **Task 6: Tests** (AC: 4, 6) — `PageShell.test.tsx` + `PageHeader.test.tsx`. Both call Testing-Library `render()` → **mandatory** `// @vitest-environment jsdom` at file top + `import { cleanup } from "@testing-library/react"; afterEach(cleanup);` (A35/A46). No hooks/i18n in the primitives → **no `next-intl` mock, no stable-ref concern (A78 N/A)** — pass plain strings/nodes as props.
  - [x] PageShell: renders `children`; default container is `max-w-7xl`; `maxWidth="4xl"` yields `max-w-4xl` (assert the class is present, proving the static-map path); the `<main>` carries `min-h-[calc(100vh-4rem)] bg-gray-50 p-4 md:p-8`; the optional `header` slot renders above `children` when provided and is absent otherwise.
  - [x] PageHeader: renders `title` as an `<h1>`; `description` `<p>` present when passed and **absent** when omitted; `actions` slot present when passed and absent otherwise.
- [x] **Task 7: DoD gate** (AC: 4) — `npm run typecheck` clean; `npx eslint frontend/src/components/layout/** frontend/eslint.config.mjs` (and the 2 touched public files) `--max-warnings=0`; `npx prettier --write` on the **new** `components/layout` files only, `--check` (read-only, hand-matched) on the pre-drifted public files (A72); `npm test -- --run` — new tests green, **full suite unchanged otherwise** (no existing spec edited). LF on every edited file (A73). `git diff --stat` minimal (the public-file diffs are comment-only).
...
**Autonomous-mode escape (A41/A43):** user directive verbatim — _"implementiere das ganze epic 30 mit allen stories ohne stopp. erst danach ein review und retro"_ — pre-declares no-stop autonomous mode; every DEC carries a story-recommended option → A32 step (d) AskUserQuestion skipped, resolutions recorded (a)/(b)/(c) below.

- **DEC-1 — root element of PageShell.** (a) **Option A** — render `<main>`. (b) Rationale: story recommends A; user autonomous-mode quote above; `<main>` is byte-for-byte what all ~73 migrated pages already emit, so a future adoption is a zero-DOM-diff swap (the nested-`<main>` a11y nit is pre-existing + out of scope). (c) Consequence: PageShell's root is `<main>`; the canonical-frame test pins `min-h-[calc(100vh-4rem)] bg-gray-50 p-4 md:p-8`.
- **DEC-2 — maxWidth handling.** (a) **Option A** — `maxWidth` prop + static full-class-string map, default `7xl`. (b) Rationale: story rec A; user autonomous; `max-w-${prop}` is invisible to the Tailwind JIT (silent full-bleed) → a literal lookup object is mandatory. (c) Consequence: `PageShellMaxWidth` union + `MAX_WIDTH_CLASS` record; the `maxWidth="4xl"` test proves the static-map path (asserts `max-w-4xl` present, `max-w-7xl` absent).
- **DEC-3 — E28 residual debt.** (a) **Option A** — resolve the two `TODO(E30)` markers comment-only. (b) Rationale: story rec A; user autonomous; PublicHeader/PublicFooter are chrome (like Header/Sidebar) — moving header logic out of `components/navigation` is a hard-constraint violation; PageShell (authenticated content frame) does not apply to the public surface. (c) Consequence: AC-7 satisfied via comment-only edits to `public-layout-shell.tsx` (lines 13-19) + `app/public/layout.tsx` (line 9); zero code/DOM change to the public shell; the existing `app/public/layout.test.tsx` (2 tests) stays green.
- **DEC-4 — eslint leaf-boundary for `components/layout`.** (a) **Option A** — add the leaf rule. (b) Rationale: story rec A; user autonomous; consistent with the existing `components/ui` leaf rule; "no longer an MVP" thoroughness guards the new shared layer against a future feature back-import. (c) Consequence: a 4th entry in `boundaryRules` forbidding `components/layout → @/features|@/app`; `features → @/components/layout` stays legal (no consumer-direction rule).

### Completion Notes List

- **Pure additive infrastructure shipped:** `PageShell` + `PageHeader` presentational primitives (no `"use client"`, no hooks, server-renderable, no `any`), a barrel `index.ts` mirroring `components/navigation/index.ts`, and 2 test files (11 tests). No page adopts the primitives in S1 (that is S2/S3/S4/S5) → zero behaviour-change risk to any existing page; the green-suite proof is "full suite unchanged + 2 new test files pass".
- **Exact-shape fidelity:** the frame (`<main className="min-h-[calc(100vh-4rem)] bg-gray-50 p-4 md:p-8"><div className="mx-auto max-w-Nxl">`) and the header block (`mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between` + `h1.text-2xl.font-bold.text-gray-900.md:text-3xl` + `p.mt-1.text-gray-600` + actions slot) reproduce `suppliers-page-content.tsx:87-103` verbatim, so S2/S5 adopt with zero DOM diff. Prettier's multi-line wrap of `<h1>{title}</h1>` is JSX-whitespace-trimmed (DOM-identical) and matches the canonical suppliers source exactly.
- **AC-Subitem completion (A29):** AC-1 ✅ (root layout untouched; no page adopts in S1) · AC-2 ✅ (zero `@/components/navigation` import; verified by eslint leaf rule + grep) · AC-3 ✅ (no provisional PageShell existed; the only prior hits were the 2 E28 TODO comments — closed via AC-7) · AC-4 ✅ (typecheck + full suite green; no existing test modified) · AC-5 ✅ (primitives + barrel at `components/layout/`) · AC-6 ✅ (typed props no-`any`, static maxWidth map, exact classes) · AC-7 ✅ (both E28 TODO markers resolved comment-only).
- **Behaviour-neutral gate fix:** `eslint.config.mjs` carried a pre-existing `import/no-anonymous-default-export` warning on `export default [...]` (present at HEAD; surfaced because Task 5 edits this file). Named the array (`const config = [...]; export default config;`) — the exported value is byte-identical; this clears the `--max-warnings=0` changed-file gate. No rule semantics changed.
- **Out-of-E30 residual surfaced (A82):** the ~73-page PageShell retrofit is NOT done in S1 (excluded by Scope Boundaries) — it became **E30-S5** (added per the user's "Add both" scope decision), so the deferred remainder is tracked as a real story, not prose tech-debt.
- **Gates:** `tsc --noEmit` clean; `npx eslint <changed> --max-warnings=0` clean; `npx prettier --write` (new files) + `--check` (modified pre-drifted) clean; `npx vitest run` = **207 files / 1955 tests green** (1944 → 1955 = +11, the 2 new suites; no existing spec touched). LF on every edited/new file (A73). `git diff --stat` minimal (comment/logical only — no whole-file reformat). `next build` deferred to the epic boundary (A58).
...
**Modified:**
- `frontend/eslint.config.mjs` (add `components/layout` leaf boundary rule — DEC-4; + behaviour-neutral named-array default-export fix)
- `frontend/src/features/public/components/public-layout-shell.tsx` (comment-only: close the E28 `TODO(E30)` marker — DEC-3)
- `frontend/src/app/public/layout.tsx` (comment-only: matching E28 `PageShell` deferral note → resolution — DEC-3)

## Change Log

- 2026-06-12: Story implemented + DoD green (Status → review). Introduced `components/layout/{PageShell,PageHeader}` (presentational, static maxWidth map, exact suppliers-frame/header reproduction) + barrel + 11 tests; added the `components/layout` eslint leaf-boundary (DEC-4); closed both E28-S4 `TODO(E30)` markers comment-only (DEC-3). DEC-1 `<main>`, DEC-2 static maxWidth map, DEC-3 resolve-E28-debt-comment-only, DEC-4 add-eslint-leaf-rule — all auto-resolved per A41 (user "ohne stopp"). Vitest 1944→1955 (+11); tsc/eslint(`--max-warnings=0`)/prettier clean; LF; minimal diffs.
