...
### Story Authoring & Dev-Story Execution Rules (from Epic-11 retro A28-A30, 2026-05-16; Epic-12 retro A31-A33, 2026-05-16; Epic-20 retro A34-A37, 2026-06-01)
...
- **Frontend Vitest + Testing Library: `afterEach(cleanup)` is mandatory in this project (A35).** Vitest in this project does NOT auto-cleanup between tests. Every `render(<Component />)` accumulates DOM in the shared `document.body`. Tests using `screen.getByText(...)` will fail with `getMultipleElementsFoundError` from the second test onwards. Every new `*.test.tsx` that uses Testing Library's `render()` must include `import { cleanup }` + `afterEach(cleanup)`. Pattern surfaced in E20-S4 LicenseFooter.test.tsx initial-failure-then-fix cycle.
...
Last Updated: 2026-06-01 (Epic-13 retro added A38-A42: Doc-bundle pattern when N stories ship into one operator-facing document / Keycloak env-var docs must grep realm JSON for `${...}` placeholders + `"bearerOnly": true` clients / Recovery commands for tools not exercised in-session must be verified or marked `[!] verify before executing` / A32 explicit autonomous-mode escape clause when all three preconditions hold / Reread-as-a-stranger pass for documentation deliverables; A28-A37 from Epic-11 + Epic-12 + Epic-20 carry forward)
